...
&nbsp;Each pipeline stage owns its implementation outside `src/mapgen/runtime.ts`



&nbsp;`generateMap()` remains the single entrypoint



&nbsp;Fixed-seed mapgen output matches current behavior in regression runs
...
Notes: Isolate or remove `generateMapLegacy` only after parity is confirmed.
