...
- Bottom command tray is the primary unit control surface: five fixed squad slots map to keys 1-5, empty slots remain visible but disabled, occupied slots select existing command-unit squads, and the active squad detail panel shows trucks, aggregate water/crew/status, stance, command modes, and dispatch formation controls. Normal 3D unit labeling is squad-level: one distinct squad marker with a leader line represents each occupied command-unit squad, while individual trucks keep only minimal selection feedback unless a focused detail view needs them.
- Base Ops is a headquarters capability on its owning town, not a separate selectable settlement identity. The HQ town banner and town panel show an `HQ` badge; only the unified HQ town panel exposes recruitment, training, persistent squad roster management, and squad dispatch. Non-HQ towns expose town commands such as evacuation but cannot deploy command squads.
- HQ squads are persistent named groups of manually assigned trucks. Truck squad membership can only change while the truck is available at HQ; partial squads may be dispatched to real map locations. Dispatch is one squad at a time through the HQ town panel and bottom command tray, with initial `Line`, `Wedge`, and `Arc` placement behavior. Squads remain where ordered until moved again; autumn rain and winter rollover issue return-to-HQ orders so trucks visibly drive home before becoming editable again.
