...
- Terrain generation uses a shared Mapgen4-inspired grid landmass core for editor previews and `terrain:elevation`, deriving dry shape, coastline intent, off-center uplands, watershed ridges, lowland basins, valleys, and drainage support fields from seeded noise, elevation redistribution, and an organic island field before sea level, ocean flooding, accurate rivers, biomes, settlements, roads, and fuel stages run. Named archetypes explicitly shape large-scale catchments before hydrology: Spine favors a central or offset watershed with side valleys, Twin Bay wraps ridges around bays and inland saddles, and Massif forms radial highland drainage with foothill and high-pocket basins. Massif and Long Spine may add one to three deterministic authoritative crag formations on separated interior highland ridge maxima before Water: each uses a ridge-aligned elongated footprint, broad peak lobes, missing or weakened lobes, readable saddles, and an asymmetric escarpment shoulder while leaving more than 90% of land untouched. Their bounded uplift is merged into the same elevation and ridge/stress fields consumed by hydrology, settlements, roads, pathing, fire, vegetation, grounding, and terrain rendering; Shelf, Twin Bay, and neutral None retain their previous terrain output until later visual review. Developer diagnostics retain the generated uplift separately from biome type so the Map Editor can report whether a map contains crags, display a `crag_uplift` locator overlay, identify hovered tiles as `landform=crag`, and show the rock-exposure, ridge, gully, and highland channels actually supplied to the mountain-rock material. This provenance is inspection-only and does not change terrain authority or gameplay behavior. Default islands should avoid square map-edge coastlines and raised cut-out slab walls: visible coastlines come from radial/noisy elevation shaping with seeded headlands, bays, shelves, and side inlets before sea level is solved, while the rectangular map border is only a narrow ocean safety guarantee. Water (`hydro:solve`) exclusively owns resolved sea level and edge-connected ocean membership. The downstream `terrain:shoreline` phase derives coast distance and beach/cliff/shelf metadata without recomputing sea level, reclassifying ocean cells, or stamping beach and shelf elevations; later mapgen and rendering stages consume that boundary unchanged. Future shoreline morphology must be authored in the upstream landform before Water calibration. The 3D renderer uses Water's supplied sea-level field for the ocean surface rather than estimating a second level from seabed elevations, keeps below-water terrain as an opaque seabed, and derives a continuous signed visual shoreline from elevation, sea level, coast class, and authoritative ocean connectivity. Only dry land classified as beach should read as playable dry sand; shelf-water seabed stays sandy but becomes cooler and darker with depth beneath continuous light blue-green ocean coverage. The single contextual ocean shader owns shoaling, wind-led waves, intermittent shore-normal breakers, foam, and beach run-up without changing simulation tile classification or adding a second surf render pass, while cliffs suppress landward swash. Coastal geometry and normal maps remain authoritative through the signed swash/shoal band; outside it, the existing coarse 1,456-triangle distant carrier supplies coverage and camera rays only, while fragments reconstruct an MdXyzX height-field hit and normal for lighting. The two fields blend across the shoreline-derived transition. Pixel footprint selects which frequencies and iterations remain without independently flattening all distant normals; as close-scale detail becomes too small, shading blends into a lower-iteration evaluation of the same MdXyzX function at broader world scale, retaining bounded strategic normal and reflection variation while the far horizon remains calmer.
- Static hydrology treats rivers, inland lakes, lake outlets, and waterfalls as one generated water network. A baked priority-flood basin pass routes seeded rainfall/runoff over the final generated terrain, detects enclosed depressions, fills accepted basins to their lowest spill elevation, and starts visible rivers from each accepted lake's calculated overflow point. Strong hill and mountain basins become lakes when rainfall, runoff, depth, ocean separation, and outlet integration are credible; downstream channels continue from lake outlets toward other lakes or the ocean. Accepted lake, river, and waterfall water tiles remain non-burnable water sources with full moisture and zero fuel. The renderer consumes one full-resolution world-space inland-water surface for river and lake contours, joins, terrain cutouts, skirts, and waterfall spans. That complete contour is immutable and authoritative in XZ: its indexed water triangles subtract the terrain triangles directly, terrain-edge intersections split both surfaces, and no original water vertex is projected onto a separately sampled terrain chord. Shared seam vertices carry source-contour and terrain-triangle provenance, a terrain closure just above authoritative water, and an indexed retained-land-material skirt extending just below it. A narrow fully submerged guard strip extends waterward from closed skirt bottoms in the same terrain buffers and draw call to cover raster/depth pinholes; contour winding supplies its interior direction, mitered joints maintain the minimum overlap, and river-mouth openings emit neither skirt nor guard. Retained terrain edges are split at every collinear cutout vertex before triangulation so adjacent cells share segmentation. Procedural mountain-rock detail remains fragment-stage color, bump/normal, roughness, strata, and fracture shading; geometric rock vertex morphing is disabled because it cannot remain watertight across the cutout mesh's T-junction topology. Developer isolation modes can render terrain, skirts, inland water, or uncovered background in ownership colors, and hover diagnostics preserve original versus rendered coordinates instead of reporting a post-snap value as source error. Geometric water displacement fades to zero across the first bank cell while normal-map, flow, lake-calmness, rapid, waterfall, and animated crest effects remain active; calm banks do not receive a continuous static white foam rim. At river mouths, the contour reaches the authoritative ocean edge, omits the outlet-facing terrain skirt, and uses a short render-only overlap to fade river flow, foam, color, and coverage into the existing ocean waves without changing either water mask. Provisional waterfall steps may shape carving, but exported waterfall markers are classified deterministically from final river/lake surfaces after lake absorption and outlet relocation, without changing topology, beds, elevations, or authoritative water surfaces.
...
- Terrain editor previews prioritize instant feedback for landform height, surface detail, and water controls using the same fast landmass core as `terrain:elevation`; Rivers, Biomes, and later final-quality stages are not started until their step is selected, then advance the current preview session instead of restarting earlier completed stages. Landform owns height controls such as Relief, Ruggedness, and Max height; their player/editor sliders are locked to 75-100%, 75-100%, and 50-100% respectively, including randomized slider values. Water exposes Land mass as the primary coastline coverage control and sea-level bias as an advanced calibration override. Production recipes normalize Land mass to 50-70%, Max height to 40-100%, and border-water falloff to 40-100%; sea-level bias adjusts the calibrated land target by at most two percentage points, keeping effective land coverage within 48-72%, while the exact map perimeter remains authoritative ocean.
