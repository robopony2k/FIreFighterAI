...
- FX Lab contains isolated grass-fidelity prototypes that combine authoritative grass coverage and rendered terrain height with the gameplay camera, scene depth, wind, and lighting. The aggressively optimized Volume Clumps profile uses conservative distance and canopy rejection, projected-size filtering, continuously interpolated packed height at occupied samples, strict non-grass exclusion with inward-feathered grass-edge height and density, selective steep-slope work, per-ray cached fields, a 60%-resolution layer with stable bilinear reconstruction, slower intermittent gusts, and 96/64/40 projected sampling. March Work visualizes coverage-weighted grass evaluation, final local grass length is capped at 0.25, and separate wind-response and wind-speed controls allow motion shimmer to be isolated. A separately selectable WebGL2 PCG SDF Blades comparison adapts the supplied implicit-map raymarch to the same terrain, curing controls, wind, and compositor with actual blade-scale dimensions, a shallow 64-step ceiling, and an explicit Volume Clumps fallback. Campaign rendering remains unchanged until supplied live captures approve appearance, occlusion, distance behavior, motion stability, and cost.
...
- Static hydrology is a single linear pass over erosion output. Hydrology intensity thresholds the existing log-normalized flow accumulation into river cells, rasterizes diagonal receiver links through one orthogonal connector, and gives every river a shallow bed and water surface derived directly from that cell's eroded elevation. River surfaces are not cumulatively stepped, rerouted, widened into gameplay cells, or repaired after generation. Lakes are the largest connected cells whose depression depth was already found by erosion's priority flood; Hydrology and basin tendency only lower the depth needed for those cells to qualify. The River stage does not run a second flood solve, rainfall/catchment rejection model, footprint promotion, overflow search, waterfall classifier, or iterative adjacency cleanup. Rivers and lakes remain static non-burnable water sources, and the existing renderer consumes their authoritative masks and local surfaces for contours and terrain cutouts.
