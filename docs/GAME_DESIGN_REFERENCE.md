...
- Each year has one deterministic, seed-jittered mid-autumn rain event. It is brief and reads as a 3D ShaderToy-inspired weather front: bounded ray-marched storm clouds in the seasonal sky plus screen-space rain aligned to wind direction relative to the camera, with wet dimming, mist, and sheen. Two coherent generated inputs span all seasons: a rotated low-frequency 2D weather field defines large separated footprints, while a padded 32³ RGBA atlas supplies continuous trilinearly interpolated 3D broad density, billows, detail, and erosion. A vertical layer envelope constrains altitude without calculating a heightfield top, allowing volume noise to form differently sized rounded crowns and broken silhouettes. Spring and especially summer distribute fair-weather coverage across fewer, larger, locally dense cumulus bodies with broad readable blue-sky gaps; winter produces broader cool formations, and rain fronts form denser low dark decks with eroded billow edges, shaded interiors, and restrained sun-facing highlights. Rays travel through a defined cloud base and top, so formations show parallax and depth rather than appearing painted onto the sky dome. Stable world-seeded weather time slowly deforms the internal volume while the same prevailing and seasonal direction field used by gameplay wind carries formations visibly with the wind at normal speed; short gusts, instantaneous direction changes, and rain-event seed changes do not reproject or teleport them during accelerated play. Sky color, cloud cover, cloud tone, and open-ocean color share one smooth seasonal weather mood: summer is clearer and bluer, winter is greyer and heavier, autumn is muted without being stormy by default, and rain/storm states visibly darken both sky and ocean. The existing single ocean shader also consumes the same wind and active-rain context: wind and rain strengthen wave motion, intermittent shoreline breakers, and short beach swash while reducing shallow-water clarity; season alone changes palette and lighting rather than wave energy. Cloud, rain, and water animation are driven by simulation career time, so they advance with the calendar and freeze when the game is paused. The event clears any remaining active fires as weather without awarding firefighter suppression credit or restoring burned fuel.
