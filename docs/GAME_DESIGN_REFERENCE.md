...
- Unit and vehicle movement should consume the full movement budget implied by current simulation speed, terrain movement costs, and character/training/progression speed upgrades. Rendering should interpolate from the previous committed position to the current committed position every frame, so high-speed or upgraded movement reads as continuous travel instead of one-tile step pauses.
