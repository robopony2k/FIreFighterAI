...
export const sampleFireClimate = (state: WorldState, careerDay: number): FireClimateSample => {
  ...
  const climateMoisture =
    DEFAULT_MOISTURE_PARAMS.Mmin + (DEFAULT_MOISTURE_PARAMS.Mmax - DEFAULT_MOISTURE_PARAMS.Mmin) * (1 - climateRisk);
...
export const buildFireWeatherResponse = (sample: FireClimateSample): FireWeatherResponse => {
  ...
  const suppression = lerp(
    band.suppression[0],
    band.suppression[1],
    clamp01((1 - sample.climateRisk) * 0.75 + coolBias * 0.25)
  );
