...
const getWetnessDecayFactor = (delta: number): number => {
    const halfLife = Math.max(0.0001, SUPPRESSION_WETNESS_HALF_LIFE_DAYS);
    return Math.exp((-Math.LN2 * Math.max(0, delta)) / halfLife);
};

const applyWetnessToIgnitionThreshold = (baseIgnition: number, wetness: number, ignitionBoost: number): number =>
    (baseIgnition * (1 + SUPPRESSION_WETNESS_IGNITION_BONUS * wetness)) / Math.max(0.0001, ignitionBoost);

const getInertHeatCap = (tid: number, fuelValue: number): number => {
    if (fuelValue > 0.02) {
...
export function runFireKernel(
    state: WorldState,
    rng: RNG,
    options: FireKernelStepOptions,
    hooks: FireKernelHooks
): FireKernelResult {
    const {
        delta,
        spreadScale,
    ...
    const isIgnitableTypeId = (tid: number): boolean =>
        tid !== TYPE_WATER &&
            tid !== TYPE_ASH &&
            tid !== TYPE_FIREBREAK &&
            tid !== TYPE_BEACH &&
            ...
            tid !== TYPE_BARE &&
            tid !== TYPE_ROAD;
    const hasNeighborFireAt = (x: number, y: number): boolean => {
        if (fireQuality === 0) {
    ...
    const getExposureFactorsAt = (
        x: number,
        y: number
    ): { exposedFuel01: number; support01: number; hasExposure: boolean } => {
    ...
    const addHeat = (idx: number, value: number): void => {
        if (value <= 0) {
            return;
        }
    ...
    const addTerrainHeat = (sourceIdx: number, targetIdx: number, value: number, distanceTiles: number): void => {
        if (value <= 0) {
            return;
        }
    ...
    const addRangedTerrainHeat = (
        ...
        localWind: TerrainAdjustedWind,
        heatReleaseValue: number,
    ...
    const hasRangedFireExposureAt = (x: number, y: number): boolean => {
            ...
            for (let x = minX; x <= maxX; x += 1, idx += 1) {
                ...
                if (burning) {
                    ...
                    if (state.tiles[idx].type === "house" && !state.tiles[idx].houseDestroyed) {
                        hooks.damageHouse({ idx, damage01: clamp(fireValue * 0.48 + overheatFactor * 0.24, 0, 1) });
                        result.houseDamageEvents += 1;
                    }
                    if (fuelValue <= 0.02 && tid !== TILE_TYPE_IDS.ash) {
                        const tile = state.tiles[idx];
                        if (tile.type === "house" && !tile.houseDestroyed) {
                            hooks.destroyHouse({ idx, x, y });
                            result.houseLossEvents += 1;
                        }
                        if (tile.type === "house") {
                            tile.fuel = 0;
                            hooks.clearBurnedVegetation({ idx });
                            result.vegetationBurnoutEvents += 1;
