...
export const createFireAnchorResolver = ({
  ...
  const offset = normalSampleOffset ?? (terrainSurface ? Math.max(0.05, terrainSurface.step * 0.35) : 0.35);
  ...
  const resolveGroundAnchor = (tileIndex: number, tileXIndex: number, tileYIndex: number): ResolvedFireAnchor => {
