...
export const emitStageSnapshot = async (ctx: MapGenContext, phase: MapGenDebugPhase): Promise<void> => {
  ...
  await ctx.debug.onPhase({
    ...
    archetypeUplift: ctx.archetypeUpliftMap ? Float32Array.from(ctx.archetypeUpliftMap) : undefined,
    flowAccumulation: ctx.flowAccumulationMap ? Float32Array.from(ctx.flowAccumulationMap) : undefined,
    erosionWear: ctx.erosionWearMap ? Float32Array.from(ctx.erosionWearMap) : undefined,
    erosionDeposit: ctx.erosionDepositMap ? Float32Array.from(ctx.erosionDepositMap) : undefined,
    rockExposure: ctx.rockExposureMap ? Float32Array.from(ctx.rockExposureMap) : undefined,
    ...
    windExposure: ctx.windExposureMap ? Float32Array.from(ctx.windExposureMap) : undefined,
    leeShelter: ctx.leeShelterMap ? Float32Array.from(ctx.leeShelterMap) : undefined,
    terrainCurvature: ctx.terrainCurvatureMap ? Float32Array.from(ctx.terrainCurvatureMap) : undefined,
    drainage: ctx.drainageMap ? Float32Array.from(ctx.drainageMap) : undefined,
    vegetationCluster: ctx.vegetationClusterMap ? Float32Array.from(ctx.vegetationClusterMap) : undefined,
    vegetationSiteQuality: ctx.vegetationSiteQualityMap ? Float32Array.from(ctx.vegetationSiteQualityMap) : undefined,
