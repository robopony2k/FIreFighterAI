...
export function setRoadAt(state: WorldState, rng: RNG, x: number, y: number): void {
...
export function canRoadTraverse(state: WorldState, x: number, y: number, start: Point, end: Point): boolean {
  if (!inBounds(state.grid, x, y)) {
    return false;
  }
...
export function findRoadPath(state: WorldState, start: Point, end: Point): Point[] {
...
export function carveRoad(state: WorldState, rng: RNG, start: Point, end: Point): boolean {
