export type ClimateParams = {
  ...
  noiseAmp: number;
  heatwavesPerYear: number;
};

export type MoistureParams = {
  Mmin: number;
  Mmax: number;
  Tdry0: number;
  Tdry1: number;
...
export const VIRTUAL_CLIMATE_PARAMS: ClimateParams = {
  ...DEFAULT_CLIMATE_PARAMS,
  seasonLen: 360,
  peakDay: 240
};

export const DEFAULT_MOISTURE_PARAMS: MoistureParams = {
  Mmin: 0.05,
  Mmax: 0.35,
  Tdry0: 18,
  Tdry1: 38,
...
export function peakCurve(day: number, peakDay: number, seasonLen: number): number {
...
export function ambientTemp(day: number, year: number, worldSeed: number, params: ClimateParams): number {
...
export function moistureStep(M: number, T: number, params: MoistureParams): number {
...
export function coolCellTemp(Tcell: number, Tenv: number, dtSeconds: number, params: CoolingParams): number {
...
export function buildClimateForecast(
  worldSeed: number,
  year: number,
  ...
  for (let day = 1; day <= days; day += 1) {
    ...
    const ignitionNorm = clamp(
      (ignitionMultiplier - CLIMATE_IGNITION_MIN) / (CLIMATE_IGNITION_MAX - CLIMATE_IGNITION_MIN),
      0,
      1
    );
...
export function buildClimateTimeline(
  ...
  params: ClimateParams,
  moisture: MoistureParams
): { daysPerYear: number; totalDays: number; risk: Float32Array } {
...
export function debugClimateChecks(worldSeed: number, params: ClimateParams, moisture: MoistureParams): void {
  if (!DEBUG_CLIMATE) {
    return;
  }
  ...
  console.log(
    `[CLIMATE] deterministic=${Math.abs(t1 - t2) < 1e-6} diffSeed=${Math.abs(t1 - t3) > 1e-6} peak=${peak.toFixed(
      2
    )} early=${early.toFixed(2)} late=${late.toFixed(2)} M=${M.toFixed(3)}`
  );
}
