...
function getTileFromPointer(state: WorldState, canvas: HTMLCanvasElement, event: MouseEvent): { x: number; y: number } | null {
...
export function bindUI(
  ...
  const isInputLocked = (): boolean => state.phase === "growth" || state.overlayVisible;
  ...
  ui.rosterList.addEventListener("click", (event) => {
    ...
    const item = target.closest(".roster-item") as HTMLElement | null;
