...
const getTerrainCost = (state, idx) => MOVE_TERRAIN_COST[state.tiles[idx].type] ?? 1;
const getSlopeFactor = (fromElev, toElev) => {
...
export const getMoveSpeedMultiplier = (state, fromX, fromY, toX, toY) => {
...
const getMoveCost = (state, fromX, fromY, toX, toY, baseCost) => {
    const fromIdx = indexFor(state.grid, fromX, fromY);
    const toIdx = indexFor(state.grid, toX, toY);
    const terrain = getTerrainCost(state, toIdx);
    const slope = getSlopeFactor(state.tiles[fromIdx].elevation, state.tiles[toIdx].elevation);
