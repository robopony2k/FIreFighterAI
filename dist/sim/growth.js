...
let canopySnapshot = null;
let forestSnapshot = null;
let snapshotSize = 0;
let lastLoggedYear = 0;
function ensureSeedSnapshots(state) {
...
function getSeedPressure(state, x, y, canopyValues, forestValues) {
    let seedSum = 0;
...
function getWaterFactor(waterDist) {
...
function logGrowthMetrics(state) {
    let ashCount = 0;
    let grassCount = 0;
    let forestCount = 0;
    ...
    console.log(`Year ${state.year} growth: ash ${ashCount} grass ${grassCount} forest ${forestCount} canopyByWaterDist [0-4:${bandAverages[0]} 5-9:${bandAverages[1]} 10-14:${bandAverages[2]} 15-19:${bandAverages[3]} 20+:${bandAverages[4]}]`);
}
